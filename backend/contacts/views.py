...
class TagViewSet(viewsets.ModelViewSet):
    """
    CRUD for a user's own Tags — see contacts/models.py's Tag docstring for
    what tags are for. Deleting a tag just removes it from any contacts
    that had it (standard M2M cascade) — it never deletes the contacts
    themselves.
    """
...
class SegmentViewSet(viewsets.ModelViewSet):
    """
    CRUD for a user's own Segments — see contacts/models.py's Segment
    docstring for what a segment is and how its membership is computed.
    """
    ...
    def get_queryset(self):
        return Segment.objects.filter(owner=self.request.user).prefetch_related("tags", "lists")
    ...
    def contacts(self, request, pk=None):
        ...
        Preview of which contacts currently match this segment's criteria —
        lets the UI show "this segment currently matches 42 contacts" and
        list them, before ever using the segment on a campaign.
        """
        from .serializers import ContactSerializer

        segment = self.get_object()
        queryset = segment.matching_contacts_queryset().order_by("id")
        page = self.paginate_queryset(queryset)
        serializer = ContactSerializer(page, many=True, context=self.get_serializer_context())
...
class ContactViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ["status", "lists", "tags"]
    search_fields = ["first_name", "last_name", "email", "phone"]
    ordering_fields = ["created_at", "email", "first_name", "last_name"]

    def get_queryset(self):
        return Contact.objects.filter(owner=self.request.user).prefetch_related("lists", "tags")
